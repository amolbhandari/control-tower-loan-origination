...
export default function Loans() {
  ...
  const filteredLoans = useMemo(() => {
    ...
    return rows.filter((loan) => {
      if (sourceFilter) {
        if ((loan as unknown as { data_source?: string | null }).data_source !== sourceFilter) return false;
      }
  ...
  }, [loans, createdDaysParam, lockExpiresDaysParam, sourceFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      {sourceFilter && (
        <Alert className="py-2 border-blue-300 bg-blue-50/60 dark:border-blue-700 dark:bg-blue-950/20">
          <Link2 className="h-4 w-4 text-blue-600" />
          <AlertDescription className="flex items-center justify-between gap-2 ml-2">
            <span className="text-sm">
              Showing loans synced from{' '}
              <span className="font-semibold capitalize">{sourceFilter === 'calyx_point' ? 'Calyx Point' : sourceFilter === 'bytepro' ? 'BytePro' : sourceFilter}</span>.
            </span>
            <button
              className="text-xs underline text-blue-700 dark:text-blue-400 hover:opacity-80"
              onClick={() => setSearchParams((p) => { const n = new URLSearchParams(p); n.delete("source"); return n; }, { replace: true })}
            >
              Clear filter
            </button>
          </AlertDescription>
